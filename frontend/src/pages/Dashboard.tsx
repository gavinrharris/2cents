...
const Dashboard = () => {
  ...
  const { data: spendingTrend, isLoading: trendLoading } = useQuery({
    queryKey: ["reports", "spending-trend"],
    queryFn: () => reportsApi.getSpendingTrend(),
  });
  const { data: dailyTips } = useQuery({
    queryKey: ["reports", "daily-tips"],
    queryFn: () => reportsApi.getDailyTips(),
  });
  const tip = dailyTips?.length ? dailyTips[Math.floor(Math.random() * dailyTips.length)] : "Set up automatic transfers to savings on payday.";
          ...
          <Card>
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-bold">
                {statsLoading ? "…" : `$${(stats?.monthlyIncome ?? user?.monthlyIncome ?? 0).toLocaleString()}`}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-bold">
                {statsLoading ? "…" : `$${(stats?.estimatedMonthlyExpenses ?? 0).toLocaleString()}`}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              ...
              <p className="text-2xl font-bold text-accent">
                {statsLoading ? "…" : `${stats?.savingsRate ?? 0}%`}
              </p>
            </CardContent>
          </Card>
        </div>
